 ...
 *
 * Usage:
 *   node --experimental-strip-types validate-marketplace.ts <path-to-marketplace.json>
 */
...
function main() {
  ...
  marketplace.plugins.forEach(
    (p: Record<string, unknown> | null, i: number) => {
  ...
  if (errors.length) {
    console.error(
      `ERROR: ${filePath} has ${errors.length} validation error(s):`,
    );
